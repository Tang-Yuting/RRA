...
class TD3(OffPolicyAlgorithm):
    ...
    def post(self, tau, recursive_type: Any) -> th.Tensor:
        if recursive_type == "dsum":
            ...
            # print("tau", tau)
            tau_dsum, tau_dmax = tau[..., 0], tau[..., 1]
            # print("sum", tau_dsum, "max", tau_dmax)
    ...
    def train(self, gradient_steps: int, batch_size: int = 100) -> None:
        ...
        for _ in range(gradient_steps):
            ...
            with th.no_grad():
                ...
                # next_tau_values = th.cat(self.critic_target(replay_data.next_observations, next_actions), dim=1)
                # # print("next_tau_values1", next_tau_values.shape)
                # next_tau_values, _ = th.min(next_tau_values, dim=1, keepdim=True)
                # target_tau_values = replay_data.rewards + (1 - replay_data.dones) * self.gamma * next_tau_values
                # # print("next_tau_values2", next_tau_values.shape)

                tau_1_next, tau_2_next = self.critic_target(replay_data.next_observations, next_actions)  # (batch_size, 3)
                # print("tau_next", tau_1_next.shape, tau_2_next.shape)
                ...
                # print("tau_next_update", tau_1_next_update.shape, tau_1_next_update.shape)
                target_tau_values = th.min(tau_1_next_update, tau_2_next_update)
                # print("target_tau_values", target_tau_values.shape, target_tau_values)

            # Get current Q-values estimates for each critic network
            current_tau_values = self.critic(replay_data.observations, replay_data.actions)
            # print("current_tau_values", current_tau_values)
            # print("target_tau_values", target_tau_values)
            # for i, current_tau in enumerate(current_tau_values):
            #     print(f"Critic {i}: shape = {current_tau.shape}, value = {current_tau}")
            # Compute critic loss
            # critic_loss = sum(F.mse_loss(current_tau, target_tau_values) for current_tau in current_tau_values)
