...
class BasePolicy(BaseModel, ABC):
    ...
    @abstractmethod
    def _predict(self, observation: PyTorchObs, extend_state: th.Tensor, deterministic: bool = False) -> th.Tensor:
        """
    ...
    def predict(
        ...
        extend_state: Optional[np.ndarray] = None,
        ...
        extend_state_tensor = th.tensor(extend_state)
        if extend_state_tensor.dim() == 1:
            extend_state_tensor = extend_state_tensor.unsqueeze(-1)
        if extend_state_tensor.dim() == 0:
            extend_state_tensor = extend_state_tensor.unsqueeze(0)

        with th.no_grad():
            actions = self._predict(obs_tensor, extend_state_tensor, deterministic=deterministic)
...
class ActorCriticPolicy(BasePolicy):
    ...
    def original_forward(self, obs: th.Tensor, deterministic: bool = False) -> tuple[th.Tensor, th.Tensor, th.Tensor]:
        """
    ...
    def forward(self, obs: th.Tensor, extend_state: th.Tensor, deterministic: bool = False) -> tuple[th.Tensor, th.Tensor, th.Tensor]:
        """
        Forward pass in all the networks (actor and critic)
        ...
        obs_extend = th.cat([obs, extend_state.unsqueeze(-1)], dim=-1)
        features = self.extract_features(obs_extend)
        if self.share_features_extractor:
            latent_pi, latent_vf = self.mlp_extractor(features)
        else:
            pi_features, vf_features = features
    ...
    def _predict(self, observation: PyTorchObs, extend_state: th.Tensor, deterministic: bool = False) -> th.Tensor:
        ...
        :return: Taken action according to the policy
        """
        return self.get_distribution(observation, extend_state).get_actions(deterministic=deterministic)
    ...
    def max_evaluate_actions(self, obs: PyTorchObs, extend_state: th.Tensor, actions: th.Tensor) -> tuple[th.Tensor, th.Tensor, Optional[th.Tensor]]:
        """
        Evaluate actions according to the current policy,
        given the observations.
    ...
    def get_distribution(self, obs: PyTorchObs, extend_state: th.Tensor) -> Distribution:
        """
        Get the current policy distribution given the observations.

        :param obs: Observation
        :param extend_state: Extended state variable
        :return: the action distribution.
        """
        if extend_state.dim() == 1:
            extend_state = extend_state.unsqueeze(-1)
        if extend_state.dim() == 0:
            extend_state = extend_state.unsqueeze(0)
        extend_state = extend_state.to(obs.device)

        obs_extend = th.cat([obs, extend_state], dim=-1)  # (batch_size, obs_dim + 1)

        features = super().extract_features(obs_extend, self.pi_features_extractor)
        latent_pi = self.mlp_extractor.forward_actor(features)

        return self._get_action_dist_from_latent(latent_pi)

    def predict_values(self, obs: PyTorchObs, extend_state: th.Tensor) -> th.Tensor:
        """
        ...
        features = super().extract_features(obs_extend, self.vf_features_extractor)
...
class CustomActorCriticPolicy(ActorCriticPolicy):
    def __init__(
        ...
        self.features_dim = self.features_dim + 1
