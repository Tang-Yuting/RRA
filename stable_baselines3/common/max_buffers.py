...
class BaseBuffer(ABC):
    ...
    ) -> Union[ReplayBufferSamples, MAXRolloutBufferSamples]:
        """
        :param batch_inds:
        :param env:
        :return:
...
class ReplayBuffer(BaseBuffer):
    ...
    def __init__(
        ...
        self.extend_state = np.zeros((self.buffer_size, self.n_envs), dtype=np.float32)
...
class RolloutBuffer(BaseBuffer):
    ...
    def reset(self) -> None:
        ...
        # self.max_returns = np.zeros((self.buffer_size, self.n_envs), dtype=np.float32)
    ...
    def compute_returns_and_advantage(self, policy, last_values: th.Tensor, dones: np.ndarray, recursive_type: Any) -> None:
        ...
        if recursive_type == "sum":
            ...
            for step in reversed(range(self.buffer_size)):
                ...
                max_value = self.gamma * max(self.extend_state[step], next_values * next_non_terminal)
    ...
    def get(self, batch_size: Optional[int] = None) -> Generator[MAXRolloutBufferSamples, None, None]:
        assert self.full, ""
    ...
    def _get_samples(
        ...
        data = (
            ...
            self.extend_state[batch_inds].flatten(),
            # self.max_returns[batch_inds].flatten(),
        )
        return MAXRolloutBufferSamples(*tuple(map(self.to_torch, data)))
